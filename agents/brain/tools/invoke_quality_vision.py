"""Lambda invoke wrapper for Quality Vision Manager.

Invokes the factorymind-quality-vision-manager Lambda function
...
def invoke_quality_vision(
    ...
    """Invoke the Quality Vision Manager Lambda function.
    ...
    Returns:
        Response payload from the Quality Vision Manager.
    ...
    logger.info(
        "invoking_quality_vision_manager",
    ...
    if response.get("FunctionError"):
        logger.error(
            "quality_vision_invocation_failed",
            function_name=FUNCTION_NAME,
            error=response_payload,
            elapsed_ms=elapsed_ms,
        )
        raise RuntimeError(
            f"Quality Vision Manager invocation failed: {response_payload}"
        )

    logger.info(
        "quality_vision_invocation_complete",
        function_name=FUNCTION_NAME,
        elapsed_ms=elapsed_ms,
    )
